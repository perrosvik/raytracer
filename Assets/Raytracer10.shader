...
Shader "Unlit/Raytracer10"
{
	...
	static const sphere WORLD[NUMBER_OF_SPHERES] = {
		{ vec3(0.0, 0.0, -1.0), 0.5, false, false, vec3(0.1, 0.2, 0.5), 1.0, 0.0},
		{ vec3(0.0, -100.5, -1.0), 100.0, false, false, vec3(0.8, 0.8, 0.0), 1.0, 0.0},
		{ vec3(1.0, 0.0, -1.0), 0.5, true, false, vec3(0.8, 0.6, 0.2), 0.3, 0.0},
		{ vec3(-1.0, 0.0, -1.0), 0.5, false, true, vec3(0.0, 0.0, 0.0), 1.0, 1.5},
		{ vec3(-1.0, 0.0, -1.0), -0.45, false, true, vec3(0.0, 0.0, 0.0), 1.0, 1.5}
		// Sphere format: { vec3 center, float radius, bool metal, bool dielectric, vec3 albedo, float fuzz, floatr ref_idx }
	};
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		camera cam = camera::from(vec3(-2,2,1), vec3(0,0,-1), vec3(0,1,0), 45, 4.0 / 2.0);
